...
zz=rrfData['RestaurantType'].value_counts()
...
# dendrogram(linkage(distance_matrix))
...
df['labels']=labels
# transformed_downsampled = df.sample(10000)
...
xxxx=analyzableDataDownSampled.groupby("cluster").agg({'GrantAmount':['count', 'mean', max, min], 'HH_Income':['mean']})
xxxx.to_csv('data_dissection.csv')
